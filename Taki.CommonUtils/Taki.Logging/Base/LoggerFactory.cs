...
namespace Taki.Logging
{
    ...
    public class LoggerFactory
    {
        ...
        public static ILogger Create()
        {
            ...
            return new Logger4Helper(); //按照可扩展规则，本来这里不能出现特定的实例，但是为了解决 LoggerFactory.CreateLog().Error(ex); 当 LoggerFactory.CreateLog() 为空时，那就成了null.Error(ex) 会报异常，所以这里就默认返回Log4net的实例。当然LoggerFactory.CreateLog()?.Error(ex);可以解决问题，当时你不能保证每个人都这么写。而且 ?. 必须4.5以上版本 C#6.0 的语法
        }
